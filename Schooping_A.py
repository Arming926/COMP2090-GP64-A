...
class Member(User):
    ...
    def apply_to_group(self, group_list):
        # TO CHECK IF YOU ARE IN GROUP OR NOT!!
        ...
        try:
            ...
            # TO CHECK IF YOU REQUESTED BEFORE
...
class Leader(Member): # Inheritance
    def __init__(self, name, password):
        super().__init__(name, password)
        self.managed_group = None

    def show_menu(self): # Polymorphism
        print(f"\n--- 組長 {self.name} 的管理選單 ---")
        print("1. 審核加入申請")
        print("2. 查看小組成員名單")
        print("3. 解散小組")
    ...
    def approve_applicant(self, target_member, my_group):
        if target_member.joined_group_obj is not None:
            print(f"⚠️ {target_member.name} 已經加入其他小組，無法重複加入。")
            my_group.pending_requests.remove(target_member) 
            return False
            
        my_group.members.append(target_member)
        target_member.joined_group_obj = my_group
        my_group.pending_requests.remove(target_member)
        return True
...
class Group:
    def __init__(self, name, description, leader, contact):
        ...
        self.__MAX_SIZE = 8 # Encapsulation
            ...
            while True:
                ...
                if role_choice == "2": 
                    if choice == "1":
                        ...
                        elif hasattr(user, 'joined_group') and user.joined_group:
                            for g in all_groups:
                    ...
                    if choice == "1": 
                        if not group.pending_requests:
                            ...
                            for applicant in list(group.pending_requests):
                                ...
                                if ans.lower() == 'y':
                                  user.approve_applicant(applicant, group)
                                elif ans.lower() == 'n':
                                  group.pending_requests.remove(applicant)
                                  print(f"已拒絕 {applicant.name} 的申請。")
                    elif choice == "2":
                        group.show_status()
                    elif choice == "3":
